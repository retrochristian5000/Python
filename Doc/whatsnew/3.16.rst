...
* :mod:`xml.dom.minidom` now conforms closer to the DOM Level 1 specification.
  ...
  rejects inserting or setting a node created by another document
  or making a node a descendant of itself,
  rejects a second element or document type child of a document
  and a notation child of a document fragment,
...
* :mod:`xml.dom.minidom` now raises :exc:`~xml.dom.InvalidCharacterErr`
  ...
  for inserting or setting a node created by another document,
  :exc:`~xml.dom.InuseAttributeErr` for setting an attribute node
  which belongs to another element,
  and :exc:`~xml.dom.HierarchyRequestErr` for inserting a node into itself
  or its descendant.
  It also rejects a second element or document type child of a document
  and a notation child of a document fragment.
  Such operations formerly succeeded
  and produced an invalid document or an endless loop.
  On the other hand, :meth:`!Element.removeAttribute` and
  :meth:`!Element.removeAttributeNS` no longer raise
...
* Add :c:func:`PyUnicodeWriter_WriteRawStr` function to write a string into
  a Unicode writer. It can be used to write a :class:`str` subclass which
  overrides the ``__str__()`` method: it writes the string contents instead of
  calling the ``__str__()`` method.
  (Contributed by Victor Stinner in :gh:`150973`.)
