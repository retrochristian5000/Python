...
class ReTests(unittest.TestCase):
    ...
    def test_redefined_named_groups_backtracking(self):
        ...
        # A redefinition in the tail of a lazy repeat matches and then
        # fails; it must not clobber the capture from the body.
        m = re.search(r'(?:(?P<g>a)|x)+?(?:(?P<g>x)q|y)', 'axy')
        self.assertEqual(m.span('g'), (0, 1))
